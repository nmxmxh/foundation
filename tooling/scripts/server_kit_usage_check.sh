...
if [[ -f "$target/.foundation" && -d "$target/internal" ]]; then
  ...
  check_file_contains "server uses registry" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/registry"
  check_file_contains "server uses graceful responses" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/graceful"
  check_file_contains "server uses metadata normalization" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/metadata"
  check_file_contains "server uses HTTP API bridge" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/httpapi"
  check_file_contains "server uses security middleware" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/security"
  check_file_contains "server uses compression middleware" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/compress"
  check_file_contains "server uses observability middleware" "$target/foundation/server-kit/go/httpserver/server.go" "server-kit/go/observability"
