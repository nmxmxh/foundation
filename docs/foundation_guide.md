...
* Use `docs/gpu_practices.md` before adding WebGPU or native GPU compute. GPU lanes are for wide homogeneous batches where transfer, dispatch, and readback are amortized; SAB/WASM or transferable workers remain the default for small bounded payloads.
* GPU payloads should stay in typed bytes, columnar buffers, or arena descriptors until the owning adapter must materialize objects. Shader/kernel code must not own tenant, auth, idempotency, or command lifecycle semantics.
* Use `docs/game_runtime_practices.md` for interactive loops, canvas/WebGL/WebGPU, media, maps, native previews, and visual dashboards. Treat those surfaces like frame-budgeted systems: reduce candidates before compute, name pass boundaries, prewarm first-use pipelines, preserve stable markers, and ship quality profiles instead of scattered magic numbers.
