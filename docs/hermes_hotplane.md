...
## Benchmark Plan and Implementation Sequence

Both are complete. The `server-kit/go/hermes` package is built, benchmarked, and
in production use across the generated projects.

For current numbers, read `foundation_benchmarks.md`. For the original plan and
its promotion targets, read `info/superseded_plans.md`.
...
- **Inline TTL Cleanup**: Expiration is calculated per record and cleaned up inline during batch writes to prevent memory creep without running background threads.
