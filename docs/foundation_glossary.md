...
| **Seed tombstone** | `.foundation-seeds.ignore` in a generated project — one project-relative destination per line, recording create-mode seeds the project deliberately removed. Without it, deletion reads as "never seeded" and the next update writes the file back. Tombstoned paths are never seeded, never drift-checked, and dropped from the seed ledger. | `scaffold_manifest.md` |
