...
func (s *Server) runWSReader(ctx context.Context, conn *wsConnection) {
	for {
		...
		env.Normalize()
		// Decode the JSON payload bytes into the typed Payload object. FromJSON
		// leaves the payload in PayloadBytes; without this, untyped handlers and
		// the subscribe/unsubscribe paths (which read env.Payload directly) would
		// observe an empty payload and reject every request.
		if err := env.MaterializePayload(); err != nil {
			s.sendWSDomainError(conn, domainerr.Validation("invalid_envelope", "invalid envelope"), env.CorrelationID)
			continue
		}
...
func terminalEventType(eventType, terminal string) string {
	...
	for _, suffix := range []string{":requested", ":ack", ":success", ":failed"} {
		if before, ok := strings.CutSuffix(eventType, suffix); ok {
