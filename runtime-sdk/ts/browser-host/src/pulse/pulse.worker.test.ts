...
describe("pulse worker", () => {
  ...
  it("processes lifecycle, visibility, and watcher messages", async () => {
    ...
    const buffer = new SharedArrayBuffer(64 + EPOCH_SLOT_COUNT * Int32Array.BYTES_PER_ELEMENT);
    const epochs = new Int32Array(buffer, 64, EPOCH_SLOT_COUNT);
    ...
    send({ type: "INIT", payload: { buffer, byteOffset: 64 } });
    await vi.advanceTimersByTimeAsync(20);
    expect(epochs[IDX_RUNTIME_TICK]).toBeGreaterThan(0);
    expect(new Int32Array(buffer)[IDX_RUNTIME_TICK]).toBe(0);
    send({ type: "SET_VISIBILITY", payload: { visible: false } });
    expect(epochs[IDX_VISIBILITY_STATE]).toBe(0);
