...
28. `MinimalInfoPanel`
29. `MinimalCheckbox` (ESM-native)
30. `MinimalSwitch` (ESM-native)
31. `MinimalNumberField` (ESM-native)
32. `MinimalTabs` (ESM-native)
33. `MinimalDatePicker` (ESM-native anchored panel + Foundation calendar)
...
16. **Interaction Ownership**: checkbox, switch, number-field, tabs, and date-picker primitives are ESM-native with no headless dependency. The date picker uses an anchored panel with outside-click and Escape dismissal. Public props stay framework-neutral; do not add peers that force non-ESM code into application module graphs.
